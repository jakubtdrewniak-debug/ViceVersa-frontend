  ...
  const { mutate: launchTournament, isPending: isSubmitting } = useMutation({
    mutationFn: async () => {
      const newTournament: TournamentDto = await callApi("/tournaments", {
        method: "POST",
        body: JSON.stringify({ name, game, format, type: entryType }),
      });
    ...
    onError: (err: Error) => toast.error(`System Failure: ${err.message}`)
  });
    ...
    <form onSubmit={handleFormSubmit} className="bg-[#0f1115] text-white p-8 rounded-2xl w-full max-w-md mx-auto space-y-8 font-sans border border-[#1a1d24] shadow-2xl">
      ...
      <div className="space-y-4">
        <div className="space-y-2">
          <label className="text-gray-600 text-[9px] font-black tracking-widest uppercase ml-1">Tournament name</label>
          <input
            value={name}
            onChange={(e) => setName(e.target.value)}
            placeholder="E.G. NEON OPEN 2026"
            className="w-full bg-[#1a1d24] border border-gray-800 rounded-xl p-4 text-sm text-white font-bold outline-none focus:border-pink-500 transition-all placeholder:text-gray-800"
          />
        </div>

        <div className="space-y-2">
          <label className="text-gray-600 text-[9px] font-black tracking-widest uppercase ml-1">Game</label>
          <input
            value={game}
      ...
      <div className="space-y-4 pt-6 border-t border-gray-800/30">
        <div className="flex justify-between items-center text-[10px] font-black tracking-widest uppercase">
          <span className="text-gray-600">Active Manifest</span>
          <span className="text-pink-500">{participants.length} Participants</span>
        </div>

        <div className="space-y-2 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
          {participants.length === 0 && (
            <div className="text-[10px] text-gray-800 font-black uppercase text-center py-6 bg-[#0c0e12] rounded-xl border border-gray-800/50 italic">
              Add some players!
            </div>
          )}
          {participants.map((p) => (
        ...
        <div className="flex gap-2">
          ...
          >
            <option value="">{isLoadingData ? "SYNCING..." : `ADD ${entryType}...`}</option>
